...
export default function App() {
  ...
  const labels = {
    ...
    standingTackle: '태클', interception: '가로채기', heading: '헤더', slidingTackle: '슬라이딩 패클',
  ...
  const savePlayer = () => {
    const player = { id: Date.now(), name, season, position, currentOVR, height, weight, preferredFoot, weakFoot, age, bodyType, stats };
  ...
  const loadPlayer = (player) => {
    setName(player.name); setSeason(player.season); setPosition(player.position);
    setHeight(player.height); setWeight(player.weight); setPreferredFoot(player.preferredFoot);
    setWeakFoot(player.weakFoot); setAge(player.age); setBodyType(player.bodyType); setStats(player.stats);
  };
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="rounded-[32px] overflow-hidden border border-emerald-400/30 p-1 shadow-2xl bg-zinc-950">
              <div className="bg-black/70 backdrop-blur-xl rounded-[30px] p-6 h-full">
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 text-sm text-zinc-300">
                  <div className="bg-white/5 rounded-xl p-3 text-center"><div className="text-zinc-500 text-xs mb-1">키</div><div className="font-bold">{height} cm</div></div>
                  <div className="bg-white/5 rounded-xl p-3 text-center"><div className="text-zinc-500 text-xs mb-1">몸무게</div><div className="font-bold">{weight} kg</div></div>
                  <div className="bg-white/5 rounded-xl p-3 text-center"><div className="text-zinc-500 text-xs mb-1">왼발 / 오른발</div><div className="font-bold text-lg">{preferredFoot === 'RIGHT' ? `${weakFoot}/5` : `5/${weakFoot}`}</div></div>
                  <div className="bg-white/5 rounded-xl p-3 text-center"><div className="text-zinc-500 text-xs mb-1">나이</div><div className="font-bold">{age}세</div></div>
                  <div className="bg-white/5 rounded-xl p-3 text-center col-span-2"><div className="text-zinc-500 text-xs mb-1">체형</div><div className="font-bold">{bodyType}</div></div>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:w-2/3">
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-6">
              <div className="bg-zinc-950 rounded-2xl p-4 border border-emerald-400/10"><label className="text-sm text-zinc-400 block mb-2">선수 이름</label><input value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-zinc-900 rounded-xl p-3 outline-none" /></div>
              <div className="bg-zinc-950 rounded-2xl p-4 border border-emerald-400/10"><label className="text-sm text-zinc-400 block mb-2">시즌</label><input value={season} onChange={(e) => setSeason(e.target.value)} className="w-full bg-zinc-900 rounded-xl p-3 outline-none" /></div>
              <div className="bg-zinc-950 rounded-2xl p-4 border border-emerald-400/10"><label className="text-sm text-zinc-400 block mb-2">포지션</label><select value={position} onChange={(e) => setPosition(e.target.value)} className="w-full bg-zinc-900 rounded-xl p-3 outline-none">{Object.keys(positions).map((pos) => (<option key={pos}>{pos}</option>))}</select></div>
            </div>
                ...
                {Object.keys(stats).map((key) => (
                  <div key={key} className="bg-zinc-900 rounded-2xl p-4 border border-emerald-400/10">
                    <div className="flex justify-between items-center mb-3 gap-3">
                      ...
                      <input type="number" min="1" max="200" value={stats[key]} onChange={(e) => setStats({...stats, [key]: Number(e.target.value)})} className="w-20 bg-zinc-900 border border-emerald-400/30 rounded-lg px-2 py-1 text-right font-black text-xl text-emerald-300 outline-none appearance-none" />
                    </div>
